...
namespace EmployeeManagement.Test
{
    public class InternalEmployeeControllerTests
    {
        ...
        public InternalEmployeeControllerTests()
        {
            ...
            //mapperMock.Setup(m => m.Map<InternalEmployee, Models.InternalEmployeeDto>(It.IsAny<InternalEmployee>()))
        ...
        {
            // Arrange
            //var employeeServiceMock = new Mock<IEmployeeService>();   //moved the logic into the constructor
            //employeeServiceMock.Setup(m => m.FetchInternalEmployeesAsync())
            //    .ReturnsAsync(new List<InternalEmployee>()
            //    {
            //       new InternalEmployee("Megan", "Jones", 2, 3000, false, 2),
        ...
        // ! Assert.IsType never works on Interfaces (eg: IEnumerable) because Assert.IsType is an exact type check so it will never be true

        [Fact]
        public async Task GetInternalEmployees_GetAction_MustReturnIEnumerableOfInternalEmployeeDtoAsModelType()
        {
            // Arrange
            // use the setup from constructor
